...
For the **friend-tester build: no.** AI is **off**, no key is included, and all
three scores plus the built-in question explanations work **fully offline**.

> **Graded build only (if you were told you have the "graded" build with live
> AI):** the ✨ Assistant's answers are generated by a hosted AI service, so
> that feature **needs internet** — and the pill shows **"AI: On"** only when
> it's actually reachable. **You still never enter an API key**, and everything
> else — flashcards, practice questions, and all three scores — stays **local
> and offline**. If you're offline or the service is unreachable, the assistant
> automatically falls back to the same offline, source-based explanation, so
> nothing breaks. (Setup for the builder: `docs/AI-PROXY-SETUP.md`.)
