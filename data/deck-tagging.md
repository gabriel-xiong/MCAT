...
Import: `data/flashcards-dev-cloze.csv` (Cloze notes) and `data/flashcards-dev-basic.csv` (Basic notes) — field `text` / `back`, tags `topic:{id}`. Source of record: `scripts/build_flashcards.py` → `data/flashcards-dev.csv`.
